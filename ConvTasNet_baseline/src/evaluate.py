...
def evaluate(args):
    ...
    model = conv_tasnet_ic.TasNet.load_model(args.model_path)
    print(model)
    model.eval()
    if args.use_cuda:
        model.cuda()
    ...
    for j in range(1,3):
        ...
        # for t in range(0,1):
            ...
            name = array_name
            if wandb_active:
                wandb.init(project="ConvTasNet_baseline_experiment", entity="tatarjit-ben-gurion-university-of-the-negev",name=name)
            ...
            with torch.no_grad():
                for i, (data) in enumerate(data_loader):
                    ...
                    clean_ref_mic = clean_batch[batch_idx, ref_ids, :] # B x T
                    ...
                    estimate_source = model(padded_mixture, ref_ids) # B x 1 x T
                    
                    # loss = -si_snr(estimate_source, padded_source, debug=False)
                        
                    M,_,T = padded_mixture.shape    
                    mixture_ref = padded_mixture[batch_idx, ref_ids, :]
