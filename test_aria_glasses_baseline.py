...
if __name__ == "__main__":
    ...
    for data_type in datas:
        ...
        if simulated_steering:
            steering_name = "simV"
            steer_path = os.path.join(base_dir, "Aria_steering.mat")
        ...
        for t in range(0,1):
            ...
            net.eval()
            if smallnet:
                name = f"{data_type}_smallnet_{model_type}"
            else:
                name = data_type

            if wandb_active:
                wandb.init(
                    project=f"Lab_Experiment_{data_type}_baseline",
            ...
            for folder in tqdm(subfolders, desc=f"Processing {data_type}"):
                ...
                s1, y, s_hat = align_with_best_shift(s1, y, s_hat, measured_shift)
